...
class Front_End_Media {
    ...
    wp_register_style('frontend-css', plugins_url( '/', __FILE__ ) . 'css/style.css', array(), '1.0', 'all');
	...
	function frontend_shortcode( $args ) {
		...
		if ( current_user_can( 'upload_files' ) ) {
			...
			$markup = '<div class="wrapper-media">';
			$markup .= '<ul class="frontend-media-list"></ul>';
			$markup .= '<button class="frontend-media-button" type="button" class="button" style="position: relative; z-index: 1;">' . $str . '</button>';
			$markup .= '<div class="frontend-media-hidden" data-name="' . $data_name . '"></div>';
			$markup .= '</div>';
